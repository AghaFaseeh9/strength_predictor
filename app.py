...
def predict_custom_strength(model, label_encoders, ma, pma, pca, te, w_c_ratio, curing_days, fiber_type):
    try:
...
with st.sidebar:
    ...
    st.download_button(
        ...
        data=comparison.to_csv(index=False) if 'comparison' in locals() else "",
        file_name="sample_predictions.csv",
        mime="text/csv",
        disabled='comparison' not in locals()
    )
...
with col3:
    st.metric("Accuracy", f"{(1 - mse/100):.1%}")
...
with st.form("custom_prediction_form"):
    ...
    with col1:
        ...
        ma = st.selectbox("Mineral Admixture (MA)", label_encoders["MA"].classes_.tolist())
        te = st.selectbox("Type of Exposure (TE)", label_encoders["TE"].classes_.tolist())
        fiber_type = st.selectbox("Fiber Type", label_encoders["Fiber_Type"].classes_.tolist())
    
    with col2:
        st.subheader("Mix Proportions")
        pma = st.slider("% MA", min_value=0.0, max_value=100.0, value=10.0, step=0.1)
        pca = st.slider("% Crystalline", min_value=0.0, max_value=10.0, value=1.0, step=0.1)
    
    with col3:
        st.subheader("Curing Conditions")
        w_c_ratio = st.slider("W/C Ratio", min_value=0.2, max_value=1.0, value=0.4, step=0.01)
        curing_days = st.slider("Curing Days", min_value=1, max_value=365, value=28, step=1)
